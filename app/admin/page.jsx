...
export default async function AdminDashboard() {
  ...
  // Run all dashboard queries in parallel to cut total DB latency.
  const [pending, confirmed, paidToday, lowStock] = await Promise.all([
    db.select({ c: sql`count(*)` }).from(orders).where(eq(orders.status, 'PENDING')).get(),
    db.select({ c: sql`count(*)` }).from(orders).where(eq(orders.status, 'CONFIRMED')).get(),
    db
