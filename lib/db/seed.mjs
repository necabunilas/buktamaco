...
// Guard: refuse to wipe the catalog if real orders exist (unless FORCE=1),
// so this can never destroy live data by accident.
const orderCount = Number(
  (await client.execute("SELECT count(*) AS c FROM orders").catch(() => ({ rows: [{ c: 0 }] }))).rows[0].c
);
if (orderCount > 0 && process.env.FORCE !== '1') {
  console.error(
    `\n⚠️  Refusing to reseed: ${orderCount} order(s) exist in this database.\n` +
      `    Reseeding DELETES all products/inventory. If you really mean to, run:\n` +
